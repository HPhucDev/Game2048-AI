...
class GameGrid(Frame):
    def __init__(self):
        ...
        self.grid()
        self.master.title(
            '19110052 Nguyen Hoang Phuc - Project AI - Game 2048')
    ...
    def init_grid(self):
        ...
        scoreCell.grid(row=0,
                       column=0,
                       columnspan=4,
                       padx=GRID_PADDING,
                       pady=GRID_PADDING,
                       sticky='news')
        scoreText = Label(master=scoreCell,
                          text="Điểm nè: " + str(self.totalScore),
                          bg="#3C3738",
                          fg="#BDC0BA",
                          font=FONT)
        #scoreText.grid()
        ...
        self.mode_btn = tk.Button(text="AI đang chơi",
                                  width=BTN_WIDTH,
                                  command=self.toggle,
    ...
    def update_grid_cells(self, score):
        ...
        self.grid_cells[-1].configure(text="Điểm nè: " + str(self.totalScore))
        for i in range(GRID_LEN):
            for j in range(GRID_LEN):
        ...
        if game_state(self.matrix) == 'win':
            ll = False
            self.grid_cells[1][1].configure(text="Bạn",
                                            bg=BACKGROUND_COLOR_CELL_EMPTY)
    ...
    def key_down(self, event):
        ...
        if key in self.commands:
            self.matrix, done, score = self.commands[repr(event.char)](
                self.matrix)
            self.update_state(score, done)
    ...
    def toggle(self):
        # global mode
        if self.mode_btn.config('text')[-1] == 'AI đang chơi':
            self.mode_btn.config(text='Bạn đang chơi')
            self.mode = False
        else:
            self.mode_btn.config(text='AI đang chơi')
            self.mode = True
            self.takeBotTurn()
